    ...
    <table id="table">
      <thead>
        <tr id="row" className="thead">
          <td id="column"><span>Nom Du Type</span></td>
      ...
      <tbody>
        {TD_ToShow.map((row, irow) => {
          ...
          // For All Actions Buttons
            const id = TD_ToShow[irow]["id_ObjectType"] ? TD_ToShow[irow]["id_ObjectType"] : null;
            const rowInfo = { number: irow + 1, itemData: row, id };

          // For Btn Edit
            const EditTest = true;
            const inputs = [{ name: `nom`, alias: `Nom Du Type`, type: `text` }];
