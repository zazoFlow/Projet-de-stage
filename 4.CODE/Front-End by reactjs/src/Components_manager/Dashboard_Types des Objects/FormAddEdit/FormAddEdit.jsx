...
export default function FormAddEdit() {
  ...
  const HSubmit = (e) => {
    ...
    if (BtnSubmitEnable) {
      ...
      if (FormForAdd) {
        axios
    ["post"](`http://localhost:8000/api/ObjectType`, { dict: { ...itemData, ...UT_pk_value } })
    .then((rep) => {
        console.log(rep);
  ...
  axios["get"](`http://localhost:8000/api/Admin/${id}/ObjectTypes`)
    .then((rep) => {
        console.log(rep);
        if (!rep.data.err) {
    ...
    .catch((err) => {
      ...
      if (!FormForAdd) {
        axios
    ["put"](`http://localhost:8000/api/ObjectType/${id}`, { seters: itemData })
    .then((rep) => {
        console.log(rep);
  ...
  return (
    <form id="form-add-edit" onSubmit={HSubmit}>
      <div id="inputs">
        <Input inp={{ name: "nom", alias: "Nom Du Type", type: "text" }} itemData={itemData} />
      </div>
